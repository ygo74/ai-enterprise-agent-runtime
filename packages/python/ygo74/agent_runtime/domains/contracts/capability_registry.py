...
description, an argument schema, the security posture and the coroutine that runs
the capability.

Microsoft Agent Framework, LangChain and CrewAI can each build their tools from
this same registry, which is what keeps a set of capabilities from being written
three times. Building the agent itself is left to each framework, in its own
documented way.
"""
...
class ResultRenderer(Protocol):
    ...
    Each agent renders its own results: what a search should look like in a
    conversation is a domain decision, not a framework one. A framework adapter
...
class SkillRegistry:
    ...
    def write_skills(self) -> tuple[SkillDescriptor, ...]:
        ...
        policy evaluated for a given caller, which lives outside the registry.
        """
