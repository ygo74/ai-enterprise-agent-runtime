...
Two paths put third-party text in front of a model: a prompt built by a
capability, and a tool result handed back to an agent. Both use the fence defined
...
None of this is sufficient on its own, and saying so matters. The structural
guarantee is elsewhere: no model output can trigger a side effect without passing
the deterministic confirmation policy in
:mod:`ygo74.agent_runtime.domains.humanapproval`. A fence reduces what a model is
likely to be fooled into proposing; it is not what stops the proposal running.
"""
