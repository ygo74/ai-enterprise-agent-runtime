"""Credentials crossing an agent, and the ports that produce them.

Three rules shape this module, and they are security properties rather than
style:

1. A token never enters a ``UserContext``, a prompt, a log or a model. It is held
   in :class:`AccessToken`, whose representation is redacted, so an accidental
   ``print`` or f-string cannot leak it.
2. A token received from a caller is never forwarded to a downstream server. The
   ...
   so an agent exchanges the token instead of relaying it.
3. Verification produces an :class:`AgentPrincipal` and nothing else. An
   application therefore cannot accidentally depend on a claim it has not
   modelled.

Only ports live here. Implementations belong to a host's infrastructure layer,
which keeps a domain testable without a network and without an identity provider.
"""
...
class DelegatedTokenSource(Protocol):
    ...
    This is what keeps a token scoped to one hop. A downstream server must be
